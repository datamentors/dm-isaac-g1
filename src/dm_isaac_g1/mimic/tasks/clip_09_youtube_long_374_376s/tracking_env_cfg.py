...
"""Tracking environment config for Clip 09 YouTube long 374-376s mimic training.

Leverages unitree_rl_lab MDP functions — only the task-specific config lives here.
"""
...
# Import MDP functions from unitree_rl_lab (installed as package on workstation)
...
class RobotSceneCfg(InteractiveSceneCfg):
    """Scene configuration for mimic training."""
...
@configclass
class CommandsCfg:
    """Command: track the Clip 09 YouTube long 374-376s motion."""

    motion = mdp.MotionCommandCfg(
        asset_name="robot",
        motion_file=f"{os.path.dirname(__file__)}/clip_09_youtube_long_374_376s.npz",
...
@configclass
class ActionsCfg:
    """Action: joint position targets for all 29 DOF."""
...
class RobotEnvCfg(ManagerBasedRLEnvCfg):
    """Full environment config for Clip 09 YouTube long 374-376s mimic training."""
