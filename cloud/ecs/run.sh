...
# Service startup command — generates random password, configures and starts all web services.
# Password is printed to stdout (captured by CloudWatch), retrievable via: ./run.sh password
SERVICES_STARTUP_CMD='SVC_PASSWORD=${SERVICE_PASSWORD:-$(python3 -c "import secrets; print(secrets.token_urlsafe(16))")}; export SERVICE_PASSWORD=${SVC_PASSWORD}; if [ -f /root/.config/code-server/config.yaml ]; then sed -i "s/^password:.*/password: ${SVC_PASSWORD}/" /root/.config/code-server/config.yaml; fi; if [ -f /root/.jupyter/jupyter_server_config.json ]; then python3 -c "import json,pathlib; p=pathlib.Path(\"/root/.jupyter/jupyter_server_config.json\"); c=json.loads(p.read_text()); c[\"ServerApp\"][\"token\"]=\"${SVC_PASSWORD}\"; p.write_text(json.dumps(c,indent=2))"; fi; VNC_PASS=${SVC_PASSWORD:0:8}; mkdir -p ~/.vnc; printf "${VNC_PASS}\n${VNC_PASS}\nn\n" | /opt/TurboVNC/bin/vncpasswd >/dev/null 2>&1; echo "============================================"; echo "  SERVICE PASSWORD: ${SVC_PASSWORD}"; echo "============================================"; echo "  code-server (VS Code): http://<host>:8080"; echo "  JupyterLab:            http://<host>:8888"; echo "  video2robot UI:        http://<host>:8000"; echo "  VNC:                   <host>:5901 (pw: ${VNC_PASS})"; echo "  noVNC (browser VNC):   http://<host>:6080"; echo "============================================"; nohup code-server --bind-addr 0.0.0.0:8080 &>/tmp/code-server.log & nohup conda run --no-capture-output -n unitree_sim_env jupyter lab --ip=0.0.0.0 --port=8888 --allow-root --no-browser &>/tmp/jupyterlab.log & nohup /opt/noVNC/utils/novnc_proxy --vnc localhost:5901 --listen 6080 &>/tmp/novnc.log & nohup python3 /usr/local/bin/video2robot-server.py &>/tmp/video2robot.log & (sleep 5; for f in ~/Desktop/*.desktop; do [ -f "$f" ] || continue; chmod +x "$f"; gio set -t string "$f" metadata::xfce-exe-checksum "$(sha256sum "$f" | awk "{print \$1}")" 2>/dev/null; done; xfdesktop --reload 2>/dev/null) &'
