...
def build_flat_observation(
    ...
    1. new_embodiment (28 or 53 DOF): Uses nested state["observation.state"]
    2. unitree_g1: Uses nested state body-part keys (left_arm, right_arm, etc.)
    ...
    # Server wrappers expect a top-level "state" key.
    observation["state"] = {}

    # Check if using new_embodiment format (single concatenated observation.state key)
    if "observation.state" in state_dims:
        ...
        for key in ["left_leg", "right_leg", "waist", "left_arm", "right_arm", "left_hand", "right_hand"]:
            ...
            if state_horizon > 1:
                vals = np.repeat(vals, state_horizon, axis=1)
            state_vals = vals.astype(np.float32)
            observation["state"][key] = state_vals
            # Backward-compatible flat aliases.
            observation[f"state.{key}"] = state_vals

    # Language: include both key styles for compatibility across server wrappers.
    # Some unitree_g1 deployments expect "annotation.human.task_description";
    # others use the simpler "task" field.
    language_batch = [[language_cmd]] * batch_size
    observation["language"] = {
        "task": language_batch,
        "annotation.human.task_description": language_batch,
    }
...
def main():
    ...
    expected_video_keys = list(getattr(video_modality, "modality_keys", []) or [])
    ...
    if expected_video_keys:
        print(f"[INFO] Server video keys: {expected_video_keys}", flush=True)

    state_dims = {k: len(v["min"]) for k, v in stats[embodiment_key]["state"].items()}
    print(f"[INFO] State dimensions: {state_dims}", flush=True)

    # Pick the camera key expected by the policy server.
    policy_primary_camera_name = primary_cam.name
    if embodiment_key == "unitree_g1":
        # unitree_g1 policies are typically trained with "ego_view".
        if "ego_view" in expected_video_keys:
            policy_primary_camera_name = "ego_view"
    elif expected_video_keys and len(expected_video_keys) == 1 and primary_cam.name not in expected_video_keys:
        # Single-camera fallback: rename to the server's expected key.
        policy_primary_camera_name = expected_video_keys[0]

    if policy_primary_camera_name != primary_cam.name:
        print(
            f"[INFO] Remapping primary camera key '{primary_cam.name}' -> '{policy_primary_camera_name}' for server compatibility",
            flush=True,
        )
        ...
        while simulation_app.is_running():
            ...
            if action_buffer is None or action_step_idx >= args_cli.num_action_steps:
                ...
                observation = build_flat_observation(
                    ...
                    primary_camera_name=policy_primary_camera_name,
