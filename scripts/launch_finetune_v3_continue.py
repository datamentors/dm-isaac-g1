...
"""
Continue PnP Apple fine-tuning v3 for 30K more steps (total 40K).

v3 reached loss=0.010 at 10K steps with grad_accum=1, batch=128.
This continues from checkpoint-10000 with identical v3 settings.

Key settings (identical to v3):
...
Run on workstation:
    ...
    WANDB_API_KEY=<key> python -u /workspace/scripts/launch_finetune_v3_continue.py
"""
...
def main():
    ...
    # Training config — RESUME from v3 checkpoint-10000
    config.training.start_from_checkpoint = (
        "/workspace/checkpoints/groot-g1-pnp-apple-dex3-v3/checkpoint-10000"
    )
    config.training.optim = "adamw_torch"
    config.training.output_dir = "/workspace/checkpoints/groot-g1-pnp-apple-dex3-v3-40k"
    config.training.num_gpus = 1
    config.training.global_batch_size = 128  # Same as v3
    config.training.gradient_accumulation_steps = 1  # Same as v3 (no accumulation)
    config.training.gradient_checkpointing = True  # Same as v3
    ...
    def _patched_init(self, *args, **kwargs):
        ...
        try:
            ...
            logging.info("[v3-40k] Added EarlyStopOnLossCallback(threshold=0.001, window=50)")
        except ImportError:
            logging.warning("[v3-40k] callbacks.py not found — running without early stopping")

    OrigTrainer.__init__ = _patched_init

    print("=" * 60)
    print("[v3-40k] Continuing v3 training for 30K more steps")
    print(f"  Resume from: v3 checkpoint-10000 (loss=0.010)")
